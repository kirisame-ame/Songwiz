...
if __name__ == "__main__":
    ...
    cache.cache_all_features(midi_database_dir, window_size, stride)
    end_time_caching = time.time()

    # # Step 2: Memuat fitur dari cache dan memproses database
    # print("\n=== Langkah 2: Memuat Fitur dari Cache ===")
    database_features = music.process_midi_database(midi_database_dir, window_size, stride)

    # # Step 3: Ranking berdasarkan humming file
    # print("\n=== Langkah 3: Ranking Berdasarkan Humming File ===")
    ranked_results = music.rank_best_match(hummed_file, database_features, window_size, stride)
    end_time = time.time()
