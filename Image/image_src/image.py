...
def save_precomputed_data(save_dir, mean_image, principal_components, dataset_pca, filenames):
    """
    Save precomputed data to disk.
    """
    np.save(os.path.join(save_dir, "mean_image.npy"), mean_image)
    np.save(os.path.join(save_dir, "principal_components.npy"), principal_components)
    np.save(os.path.join(save_dir, "dataset_pca.npy"), dataset_pca)
    with open(os.path.join(save_dir, "filenames.json"), 'w') as f:
        json.dump(filenames, f)

def load_precomputed_data(save_dir):
    """
    Load precomputed data from disk.
    """
    mean_image = np.load(os.path.join(save_dir, "mean_image.npy"))
    principal_components = np.load(os.path.join(save_dir, "principal_components.npy"))
    dataset_pca = np.load(os.path.join(save_dir, "dataset_pca.npy"))
    with open(os.path.join(save_dir, "filenames.json"), 'r') as f:
        filenames = json.load(f)
    return mean_image, principal_components, dataset_pca, filenames


def retrieve_similar_images(query_image, image_database_dir, num_pca_components=50, top_n=5, target_size=(64, 64), save_dir="precomputed_data", similarity_threshold=0.7):
    """
    ...
    precomputed_files = all(
        os.path.exists(os.path.join(save_dir, fname))
        for fname in ["mean_image.npy", "principal_components.npy", "dataset_pca.npy", "filenames.json"]
    )

    # Load precomputed data or compute it if necessary
    if precomputed_files:
        mean_image, principal_components, dataset_pca, filenames = load_precomputed_data(save_dir)
    else:
    ...
    return [(filtered_results[i][0], filtered_results[i][1]) for i in top_indices]
