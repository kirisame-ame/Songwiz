...
const SongCard: React.FC<SongCardProps> = ({ image, title, artist, audio_type, audio_path, onPlay }) => {
    ...
    const getAudioPath = (audio_type: string, audio_path: string) => {
        if (audio_type === 'midi' || audio_type === 'mid') {
            return `uploads/midi/${audio_path}`;
        } else if (audio_type === 'mp3' || audio_type === 'wav') {
            return `uploads/audio/${audio_path}`;
        }
        return audio_path;
    };
    
    useEffect(() => {
        if (isPlaying && (audio_type === 'midi' || audio_type === 'mid' || audio_type === 'wav' || audio_type === 'mp3')) {
            const path = getAudioPath(audio_type, audio_path);
            fetch(path)
                .then(response => response.blob())
                .then(blob => {
                    const file = new File([blob], `audio.${audio_type}`, { type: `audio/${audio_type}` });
                    setAudioFile(file);
                })
                .catch(error => {
    ...
    return (
        <div className="rounded-lg shadow-lg overflow-hidden">
            ...
            <div className="p-2 text-center">
                ...
                <p className="text-sm text-gray-500">{artist} <span className="mx-2 " style={getAudioTypeStyle(audio_type)}>{audio_type}</span></p>
            </div>  
